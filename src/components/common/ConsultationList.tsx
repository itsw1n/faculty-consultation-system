...
export function ConsultationList({
  ...
  actions?: (item: ConsultationItem) => ReactNode
}) {
  if (!items.length) return <p className="p-8 text-center text-muted">No consultations found.</p>
  return (
    <section className="overflow-hidden rounded-xl border border-border bg-surface">
      {items.map((item) => (
        ...
        >
          <div>
            <strong>{person === 'student' ? item.student_name : item.faculty_name}</strong>
