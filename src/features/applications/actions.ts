...
export async function reviewApplications(formData: FormData) {
  ...
  const { data, error } = await (await createClient()).rpc('review_applications', { target_user_ids: userIds, decision })
  ...
  if (failures) throw new Error(`${failures} application records changed before this review. Refresh and try again.`)
  revalidatePath('/admin/users')
}
