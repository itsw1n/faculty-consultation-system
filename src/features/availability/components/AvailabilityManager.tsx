    ...
    <>
      <DialogTrigger>
        ...
        <ModalOverlay className={overlayClass} isDismissable>
          <Modal className="w-full max-w-3xl rounded-xl bg-surface p-6 shadow-dialog">
            <Dialog className="outline-none">
              {({ close }) => (
                <>
                  ...
                  <p className="mt-2 text-muted">Create an open time for student requests.</p>
                  <form
                    action={formAction}
                  ...
                  >
                    ...
                    <label className={fieldClass}>
                      Mode
                      <select className={controlClass} name="mode" required>
      ...
      >
        {slots.length === 0 ? (
          ...
          slots.map((slot) => (
            ...
            >
              <DialogTrigger>
                <Button variant="ghost" className="justify-start px-0 text-left">
                ...
                <ModalOverlay className={overlayClass} isDismissable>
                  <Modal className="w-full max-w-lg rounded-xl bg-surface p-6 shadow-dialog">
                    <Dialog className="outline-none">
                      ...
                      <dl className="mt-4 grid grid-cols-[7rem_1fr] [&>dd]:m-0 [&>dd]:border-b [&>dd]:border-border [&>dd]:p-2 [&>dt]:border-b [&>dt]:border-border [&>dt]:p-2 [&>dt]:font-bold">
                        <dt>Date</dt>
                        <dd>{slot.date}</dd>
                        <dt>Time</dt>
                        <dd>
                        ...
                        <dd>{slot.status}</dd>
                        <dt>Mode</dt>
                        <dd>{slot.mode === 'IN_PERSON' ? slot.location : 'Online'}</dd>
                      </dl>
                    </Dialog>
                  </Modal>
                </ModalOverlay>
