...
export default async function DepartmentsPage() {
  ...
  return (
    <>
      <PageHeading
        title="Departments"
        description="Manage departments used in applications and faculty discovery."
      />
      <section className="rounded-xl border border-border bg-surface p-5 shadow-card">
        <h2 className="text-xl font-bold">Add department</h2>
        <form
          action={createDepartment}
          className="mt-4 grid items-end gap-3 sm:grid-cols-[1fr_2fr_auto]"
        >
          ...
          <button className="min-h-11 rounded-lg bg-primary px-4 font-bold text-white">
            Add department
          </button>
        </form>
      </section>
      <section
        className="mt-4 grid grid-cols-[repeat(auto-fill,minmax(15rem,1fr))] gap-4"
